...
public class AACMappings implements AACPage {
		...
		try {
			...
			while ((line = eyes.readLine()) != null) {
				...
				if (lineSplit[0].charAt(0) != '>') {
					catName = lineSplit[1];
					overaching.set(lineSplit[0], lineSplit[1]);
					cats.set(catName, new AACCategory(catName));
					home.addItem(lineSplit[0], lineSplit[1]);
				} else {
					cats.get(catName).addItem(lineSplit[0].substring(1), lineSplit[1]);
				} // if/else
			} // while
			eyes.close();
	...
	public void reset() {
		try {
			categoryCurrent = cats.get("");
		} catch (Exception e) {
			System.err.println("Error: No Default");
		} // try/catch
	} // reset()
	...
	public void addItem(String imageLoc, String text) {
		categoryCurrent.addItem(text, imageLoc);
	}
